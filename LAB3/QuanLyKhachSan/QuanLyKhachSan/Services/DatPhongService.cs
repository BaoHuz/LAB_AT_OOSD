            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    foreach (var x in ds)
                    {
                        ...
                        int max = Convert.ToInt32(o);

                        if (x.SoNguoi <= 0 || x.SoNguoi > max)
                            return KetQuaXuLy.Fail($"Số người của phòng {x.SoPhong} vượt quá sức chứa tối đa ({max} người).");

                        if (PhongTrungLich(cn, tx, x.SoPhong, nhan, tra))
                            return KetQuaXuLy.Fail($"Phòng {x.SoPhong} đã bị trùng lịch đặt trong khoảng thời gian này.");
                    }
