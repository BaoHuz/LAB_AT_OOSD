...
namespace QuanLyKhachSan.Services
{
    ...
    {
        public DataTable LayPhieuDangO() => Db.Query(@"SELECT d.SoPhieuDat, k.HoTen, d.NgayNhanThucTe, d.NgayTraDuKien 
                                                       FROM PhieuDatPhong d 
                                                       JOIN KhachHang k ON d.MaKhach=k.MaKhach 
                                                       WHERE d.TrangThai=N'Đang ở' 
                                                       ORDER BY d.SoPhieuDat");

        public DataTable LayPhongTheoPhieu(string so) => Db.Query("SELECT c.SoPhong, p.DonGiaNgay FROM ChiTietDatPhong c JOIN Phong p ON c.SoPhong=p.SoPhong WHERE c.SoPhieuDat=@s", new SqlParameter("@s", so));

        public DataTable LayTienNghiPhong(string phong) => Db.Query(@"SELECT TOP 100 p.MaTienNghi, l.TenLoaiTN, t.TinhTrangHienTai 
                                                                     FROM PhieuLapDat p 
        ...
        public DataTable LayHoaDon() => Db.Query(@"SELECT h.*, k.HoTen FROM HoaDon h 
        ...
        public KetQuaXuLy LapHoaDon(string soHD, string soDat, DateTime ngay, string maNV, int soNgay)
        {
            ...
            try
            {
                ...
                Db.Execute(@"INSERT INTO HoaDon(SoHoaDon, SoPhieuDat, NgayLap, MaNV, SoNgayTinhTien, TienPhong, TienDichVu, TrangThai) 
                    ...
                    new SqlParameter("@nv", maNV), new SqlParameter("@ng", soNgay), new SqlParameter("@p", phong), new SqlParameter("@d", dv));
        ...
        public KetQuaXuLy ThanhToan(string maTT, string soHD, DateTime ngay, string hinhThuc, decimal tien)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    if (da + tien > tong)
                        return KetQuaXuLy.Fail($"Số tiền thanh toán vượt quá số tiền còn phải trả ({tong - da:N0} đ).");
        ...
        public KetQuaXuLy TraPhong(string soDat, DateTime ngayTra)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        if (!rd.Read()) return KetQuaXuLy.Fail("Chưa lập hóa đơn cho phiếu đặt phòng này.");
                        tt = Convert.ToString(rd[1]);
                    }

                    if (tt != "Đã thanh toán")
                        return KetQuaXuLy.Fail("Hóa đơn chưa thanh toán đủ số tiền. Không thể hoàn tất trả phòng.");
