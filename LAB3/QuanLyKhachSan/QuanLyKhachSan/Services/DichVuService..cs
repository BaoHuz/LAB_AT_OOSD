...
namespace QuanLyKhachSan.Services
{
    ...
    {
        public DataTable LayPhieuDangO() => Db.Query(@"SELECT d.SoPhieuDat, k.HoTen, c.SoPhong 
        ...
        public DataTable LayLichSu(string so) => Db.Query(@"SELECT p.SoPhieuSDDV, p.SoPhong, p.NgaySuDung, d.TenDV, c.SoLuong, c.DonGia, c.ThanhTien 
        ...
        public KetQuaXuLy GhiNhan(string soPhieuDat, string soPhong, DateTime ngay, string maNV, string maDV, int soLuong)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    tx.Commit();
                    return KetQuaXuLy.Ok("Đã ghi nhận dịch vụ (đã tự động cộng dồn nếu cùng dịch vụ trong ngày).");
                }
                catch (Exception ex)
                {
                    tx.Rollback();
